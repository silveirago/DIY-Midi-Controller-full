...
#define BLEMIDI 1//* coloque aqui o uC que você está usando, como nas linhas acima seguido por "1", como "ATMEGA328 1", "DEBUG 1", etc.
...
char bleMIDIDeviceName[] = {"Meu Controlador MIDI"}; // put here the name you want for your device
#endif
...
const byte BUTTON_ARDUINO_PIN[N_BUTTONS] = {5,18,19}; //* pinos de cada botão conectados diretamente ao Arduino
...
byte MESSAGE_VAL[N_BUTTONS] = {36, 38, 40};
...
const byte POT_ARDUINO_PIN[N_POTS_ARDUINO] = {4, 2}; //* pinos de cada potenciômetro conectado diretamente ao Arduino
...
byte POT_CC_N[N_POTS] = {1, 2}; // Adicione o CC NUMBER de cada pote que você quiser OU MACKIE
#endif

// - - - - - - - - - - - - - - - - - - - - - - - - - - - -

const int TIMEOUT = 300; //* Quantidade de tempo que o potenciômetro será lido após exceder o varThreshold
const byte varThreshold = 35; //* Limite para a variação do sinal do potenciômetro - Aumente se estiver usando o ESP32
// Para varThreshold eu costumo usar "8" para Arduino normal e "30" para ESP32

// coloca aqui a leitura min e max no potCState
// no potMin coloque um pouco mais e no potMax coloque um pouco menos
// IMPORTANTE:
// Arduinos comuns têm resolução de 10 bits: 0 - 1023
