...
const byte N_POTS = 1;  //* número total de potenciômetros (slide e rotativo). Número de potenciômetros no Arduino + número de potenciômetros no multiplexador 1 + número de potenciômetros no multiplexador 2...

const byte N_POTS_ARDUINO = 1;  //* número de potenciômetros conectados diretamente ao Arduino
// Se estiver usando o Arduino, declare como "A1, A2"
// Se estiver usando o ESP32, use apenas o número GPIO como "11, 10"
const byte POT_ARDUINO_PIN[N_POTS_ARDUINO] = { A0 };  //* pinos de cada potenciômetro conectado diretamente ao Arduino (não use "A" se estiver usando o ESP32, apenas o número)
...
byte POT_CC_N[N_POTS] = { 1 };  // Adicione o NÚMERO CC ou MACKIE de cada potenciômetro que você deseja
...
const byte motorDownPin[N_M_FADERS] = { 7 };   //Controle H-Bridge para fazer o motor descer
const byte motorUpPin[N_M_FADERS] = { 8 };     //Controle H-Bridge para fazer o motor subir
const byte faderSpeedPin[N_M_FADERS] = { 9 };  // pino de velocidade do motor (PWM)

byte faderSpeed[N_M_FADERS] = { 255 };  // 0-255
byte faderSpeedMin = 240;               // 0-255 - 140?
...
const int touchSendPin[N_M_FADERS] = { 16 };     // Pino de envio para o circuito de detecção de capacitância (Digital 7)
const int touchReceivePin[N_M_FADERS] = { 10 };  // Pino de recebimento para o circuito de detecção de capacitância (Digital 8)
