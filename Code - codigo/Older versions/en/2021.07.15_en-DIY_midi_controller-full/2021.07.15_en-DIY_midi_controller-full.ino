...
const byte N_BUTTONS = 7; //*  total numbers of buttons. Number of buttons in the Arduino + number of buttons on multiplexer 1 + number of buttons on multiplexer 2...
const byte N_BUTTONS_ARDUINO = 2; //* number of buttons connected straight to the Arduino
const byte BUTTON_ARDUINO_PIN[N_BUTTONS] = {16, 10}; //* pins of each button connected straight to the Arduino

#ifdef USING_MUX // Fill if you are using mux, otherwise just leave it
const byte N_BUTTONS_PER_MUX[N_MUX] = {5}; //* number of buttons in each mux (in order)
const byte BUTTON_MUX_PIN[N_MUX][16] = { //* pin of each button of each mux in order
{2, 3, 4, 5, 6}, //* pins of the first mux
// ...
};
...
const byte N_POTS = 3; //* total numbers of pots (slide & rotary). Number of pots in the Arduino + number of pots on multiplexer 1 + number of pots on multiplexer 2...

const byte N_POTS_ARDUINO = 1; //* number of pots connected straight to the Arduino
const byte POT_ARDUINO_PIN[N_POTS_ARDUINO] = {A2}; //* pins of each pot connected straight to the Arduino
