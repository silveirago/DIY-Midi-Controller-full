    ...
    if (potMoving == true) { // If the potentiometer is still moving, send the change control
      ...
      if (potMidiPState[i] != potMidiCState[i]) {
        ...
        if (isTouched[i] == true) {
...
#ifdef ATMEGA328 // use if using with ATmega328 (uno, mega, nano...)


          //#ifdef USING_REMOTE_SCRIPT // if USING Mackie protocol
          ...
          //          //Serial.println("GAHHH");
          //#endif // NOT USING_REMOTE_SCRIPT OR HIGH RES
          //


          MIDI.sendControlChange(1, potMidiCState[i], 1); // CC number, CC value, midi channel - custom cc
...
#else // not using remote script or HighRes
